...
float FuzzySystemCenter::doInferenceAndDeFuzzy(float dHorizontalDeviate, float dVerticalDistanceRemain, float leftDistanceRemain, float rightDistanceRemain)
{
    ...
    if ( fRules->leftDistanceRemain_critical(leftDistanceRemain) >= fRules->leftDistanceRemain_normal(leftDistanceRemain) )
        LDR = LDR_critical;
    if ( fRules->rightDistanceRemain_critical(rightDistanceRemain) >= fRules->rightDistanceRemain_normal(rightDistanceRemain) )
        RDR = RDR_critical;
    
    for ( vector<FuzzyRulesResult>::iterator it=vRes.begin() ; it != vRes.end() ; it++ )
    {
        FuzzyRulesResult res1 = (*it);
        WheelAngle WA = fRules->fuzzyRule_GetResultByCondition(res1.ConditionA_HD, res1.ConditionB_VDR, LDR, RDR);
        ...
        float mu_1 = fRules->getMuOfRule_ifXisA_and_YisB_atSinglePoint(WA, Z);
        ...
        while ( Z < RBound )
        {
            ...
            for ( vector<FuzzyRulesResult>::iterator it2=vRes.begin() ; it2 != vRes.end() ; it2++ )
            {
                ...
                mu_2 = fRules->getMuOfRule_ifXisA_and_YisB_atSinglePoint(fRules->fuzzyRule_GetResultByCondition(fRes2.ConditionA_HD, fRes2.ConditionB_VDR, LDR, RDR), Z);
                if ( mu_2 > alpha_2 )
                    mu_2 = alpha_2;
            }
