...
bool Initializer::loadData()
{
    ifstream road("road.txt", ios::in);
    ofstream ofsroad("trackOutput.txt", ios::out);
    ...
    for ( vector<Point>::iterator it=m_vTrackInfo.begin() ; it != m_vTrackInfo.end() ; it++ )
    {
        Point pt = (*it);
        ofsroad << pt.x << " " << pt.y << endl;
    }
